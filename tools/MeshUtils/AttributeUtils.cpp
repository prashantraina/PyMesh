...
namespace AttributeUtilsHelper {
    ...
    VectorI convert_vertex_attribute_to_face_attribute(Mesh& mesh,
                                                       const VectorI& attribute) {
        const size_t num_vertices = mesh.get_num_vertices();
        const size_t num_faces = mesh.get_num_faces();
        const size_t vertex_per_face = mesh.get_vertex_per_face();
        const size_t attr_size = attribute.size();
        ...
        const VectorI& faces = mesh.get_faces();
        Eigen::VectorXd result = Eigen::VectorXd::Zero(num_faces * stride);
        ...
        for (size_t i=0; i<num_faces; i++) {
            ...
            for (size_t j=0; j<vertex_per_face; j++) {
                result.segment(i*stride, stride) +=
                        attribute.cast<double>().segment(face[j]*stride, stride);
            }
        }
        result /= vertex_per_face;
        return result.unaryExpr(&::round).cast<int>();
    }
    ...
    VectorI convert_vertex_attribute_to_voxel_attribute(Mesh& mesh,
                                                        const VectorI& attribute) {
        const size_t num_vertices = mesh.get_num_vertices();
        const size_t num_voxels = mesh.get_num_voxels();
        const size_t vertex_per_voxel = mesh.get_vertex_per_voxel();
        ...
        const VectorI& voxels = mesh.get_voxels();
        Eigen::VectorXd result = Eigen::VectorXd::Zero(num_voxels * stride);
        ...
        for (size_t i=0; i<num_voxels; i++) {
            ...
            for (size_t j=0; j<vertex_per_voxel; j++) {
                result.segment(i*stride, stride) +=
                        attribute.cast<double>().segment(voxel[j]*stride, stride);
            }
        }
        result /= vertex_per_voxel;
        return result.unaryExpr(&::round).cast<int>();
    ...
    VectorF convert_face_attribute_to_vertex_attribute(
        ...
        const VectorI& faces = mesh.get_faces();
        if (!mesh.has_float_attribute("face_area")) {
            mesh.add_float_attribute("face_area");
        }
        const VectorF& weights = mesh.get_float_attribute("face_area");
    ...
    VectorI convert_face_attribute_to_vertex_attribute(
        ...
        Eigen::VectorXd result = Eigen::VectorXd::Zero(num_vertices * stride);
        Eigen::VectorXd result_weights = Eigen::VectorXd::Zero(num_vertices);
        for (size_t i=0; i<num_faces; i++) {
            const VectorI& face =
                    faces.segment(i*vertex_per_face, vertex_per_face);
            Float per_vertex_weight = weights[i] / vertex_per_face;
            for (size_t j=0; j<vertex_per_face; j++) {
                result_weights[face[j]] += per_vertex_weight;
                result.segment(face[j]*stride, stride) +=
                        per_vertex_weight * attribute.cast<double>().segment(i*stride, stride);
            }
        }
    ...
    VectorF convert_voxel_attribute_to_vertex_attribute(
        ...
        const VectorI& voxels = mesh.get_voxels();
        if (!mesh.has_float_attribute("voxel_volume")) {
            mesh.add_float_attribute("voxel_volume");
        }
        const VectorF& weights = mesh.get_float_attribute("voxel_volume");
    ...
    VectorI convert_voxel_attribute_to_vertex_attribute(
            Mesh& mesh, const VectorI& attribute) {
        const size_t num_vertices = mesh.get_num_vertices();
        const size_t vertex_per_voxel = mesh.get_vertex_per_voxel();
        const size_t num_voxels = mesh.get_num_voxels();
    ...
    VectorI convert_voxel_attribute_to_face_attribute(
            Mesh& mesh, const VectorI& attribute) {
        const size_t num_faces = mesh.get_num_faces();
        const size_t num_voxels = mesh.get_num_voxels();
        const size_t attr_size = attribute.size();
        ...
        std::map<Triplet, Eigen::VectorXd> per_face_values;
        for (size_t i=0; i<num_voxels; i++) {
            const VectorI& voxel = voxels.segment(
                    i*vertex_per_voxel, vertex_per_voxel);
            const Eigen::VectorXd& value = attribute.segment(
                    i*stride, stride).cast<double>();
            per_face_values[{voxel[0], voxel[1], voxel[2]}] = value;
            per_face_values[{voxel[0], voxel[1], voxel[3]}] = value;
            per_face_values[{voxel[0], voxel[2], voxel[3]}] = value;
            per_face_values[{voxel[1], voxel[2], voxel[3]}] = value;
...
VectorF AttributeUtils::convert_to_vertex_attribute(Mesh& mesh, const VectorF& attribute) {
    ...
    if (attr_size == num_vertices || attr_size == dim*num_vertices) {
        ...
        throw RuntimeError("Unknown attribute type.");
    }
}

VectorI AttributeUtils::convert_to_vertex_attribute(Mesh& mesh, const VectorI& attribute) {
    const size_t dim = mesh.get_dim();
    const size_t num_vertices = mesh.get_num_vertices();
    const size_t num_faces = mesh.get_num_faces();
    const size_t num_voxels = mesh.get_num_voxels();
...
VectorF AttributeUtils::convert_to_vertex_float_attribute_from_name(Mesh& mesh,
        const std::string& attribute_name) {
    const VectorF& attr = mesh.get_float_attribute(attribute_name);
    return convert_to_vertex_attribute(mesh, attr);
}
VectorI AttributeUtils::convert_to_vertex_int_attribute_from_name(Mesh& mesh,
                                                                    const std::string& attribute_name) {
    const VectorI& attr = mesh.get_int_attribute(attribute_name);
    return convert_to_vertex_attribute(mesh, attr);
}
...
VectorI AttributeUtils::convert_to_face_attribute(Mesh& mesh, const VectorI& attribute) {
    const size_t dim = mesh.get_dim();
    const size_t num_vertices = mesh.get_num_vertices();
    const size_t num_faces = mesh.get_num_faces();
    const size_t num_voxels = mesh.get_num_voxels();
...
VectorF AttributeUtils::convert_to_face_float_attribute_from_name(Mesh& mesh,
        const std::string& attribute_name) {
    const VectorF& attr = mesh.get_float_attribute(attribute_name);
    return convert_to_face_attribute(mesh, attr);
}
VectorI AttributeUtils::convert_to_face_int_attribute_from_name(Mesh& mesh,
                                                                  const std::string& attribute_name) {
    const VectorI& attr = mesh.get_int_attribute(attribute_name);
    return convert_to_face_attribute(mesh, attr);
}
...
VectorI AttributeUtils::convert_to_voxel_attribute(Mesh& mesh,
                                                   const VectorI& attribute) {
    const size_t dim = mesh.get_dim();
    const size_t num_vertices = mesh.get_num_vertices();
    const size_t num_faces = mesh.get_num_faces();
...
VectorF AttributeUtils::convert_to_voxel_float_attribute_from_name(Mesh& mesh,
        const std::string& attribute_name) {
    const VectorF& attr = mesh.get_float_attribute(attribute_name);
    return convert_to_voxel_attribute(mesh, attr);
}

VectorI AttributeUtils::convert_to_voxel_int_attribute_from_name(Mesh& mesh,
                                                                   const std::string& attribute_name) {
    const VectorI& attr = mesh.get_int_attribute(attribute_name);
    return convert_to_voxel_attribute(mesh, attr);
}